# ECMWF IFS GIK — daily ecFlow workflow (deployment & operations guide)

An always-on ecFlow deployment that runs the **ECMWF IFS ensemble GIK pipeline every
day**, unattended. This README is the complete guide: what it does, how it is wired,
how to operate it, and how to fix it.
...
store** on source.coop (`e4drr-project/forecasts/ecmwf_ifs_ens_aws_s3_icechunk_vd`); see
§9.
...
The executor picks the era from the date and calls this wrapper, so the daily run is
always era-correct. (Dates ≥ 2026-05-13 are `50r1`: `oper` / ref `20260513`.)
...
| Generic engine + dashboard + pyflow (this repo) | `icpac-igad/ecflow @ deployment` → `deployment/` |
...
dashboard and executor run as systemd user services (see `dashboard/systemd/*.example`);
...
- `docker/` — engine stack · `dashboard/` — monitoring UI · `pyflow/` — suite generator
- Era operationalization detail: `cno-e4drr` → `devops/lithops_cr_ecmwf_gik/ECFLOW_50R1_OPERATIONALIZATION.md`
